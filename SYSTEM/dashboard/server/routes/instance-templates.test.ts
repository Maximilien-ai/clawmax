...
async function main() {
  ...
  try {
    ...
    assert.deepEqual(caps.body.templates.operations, ['export', 'import', 'list', 'remove', 'show', 'validate', 'versions'])
    ...
    const exported = await fetch(`${base}/owned/templates/${template.id}/export`, { headers })
    assert.equal(exported.status, 200)
    assert.equal(exported.headers.get('content-type'), TEMPLATE_MEDIA_TYPE)
    assert.equal(exported.headers.get('cache-control'), 'no-store')
    assert.equal(exported.headers.get('content-disposition'), `attachment; filename="${template.id}.zip"`)
    assert.deepEqual(Buffer.from(await exported.arrayBuffer()), bytes, 'Export returns only the original validated portable bundle')
    assert.equal((await call(`other/templates/${template.id}/export`)).status, 403)
    assert.equal((await call(`owned/templates/${template.id}/export?includeSecrets=true`)).status, 400)
    ...
    assert.equal((await call(`owned/templates/${template.id}`, 'DELETE')).body.removed, true)
    assert.equal((await call('owned/templates/unknown', 'DELETE')).body.removed, true)
    ...
    assert.equal((await call(`owned/templates/${template.id}/export`)).status, 404)
    assert.equal((await call('owned/templates', 'POST', bytes)).status, 409)
    assert.equal((await call('owned/not-implemented')).status, 404)
    const indexPath = path.join(reopened.root, 'catalog.json')
    const indexBytes = fs.readFileSync(indexPath)
    fs.renameSync(indexPath, `${indexPath}.saved`)
    fs.symlinkSync(`${indexPath}.saved`, indexPath)
    assert.equal((await call('owned/templates')).status, 409)
    assert.equal((await call('owned/templates', 'POST', bytes)).status, 409)
    assert.deepEqual(fs.readFileSync(`${indexPath}.saved`), indexBytes, 'Symlink target must remain untouched')
    fs.unlinkSync(indexPath)
    fs.renameSync(`${indexPath}.saved`, indexPath)
