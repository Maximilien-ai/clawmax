...
import { runDevHostSkillAgent } from '../lib/dev-host-skill-agent'
...
function resolve(agentId: string) {
  ...
  if (!binding || binding.skills.length !== 1) return null
  const skillName = binding.skills[0].name
  ...
  const skillInstructions = readBoundFile(templateStoragePath(workspace.path, `SKILLS/custom/${skillName}/SKILL.md`), 64 * 1024)
...
export async function executeDevHostSkillChat(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (!resolved) { res.status(409).json({ error: 'Collector Skill authority unavailable' }); return }
    context = resolved
  } catch { res.status(409).json({ error: 'Collector Skill authority unavailable' }); return }
  ...
  await withRegisteredTurn(agentId, async turn => {
    ...
    try {
      const text = await runDevHostSkillAgent({ message, skillName: context.skillName,
