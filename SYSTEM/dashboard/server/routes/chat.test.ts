...
test('deriveChatError surfaces claude/droid CLI auth failures with a runtime-specific remediation message', () => {
  const claudeMessage = deriveChatError('Please run /login to authenticate.', 'anthropic', 'Claude Code')
  ...
  const droidMessage = deriveChatError('Error: not logged in to Factory.', 'openai', 'Factory Droid')
...
test('deriveChatError surfaces claude/droid session-state mismatches as a retry hint', () => {
  const alreadyInUse = deriveChatError('Error: Session ID 8db2cbb6-235b-4bdf-89e5-80c37cc0181a is already in use.', 'anthropic', 'Claude Code')
  assert(/out of sync/i.test(alreadyInUse), `Unexpected already-in-use message: ${alreadyInUse}`)

  const notFound = deriveChatError('No conversation found with session ID: 8db2cbb6-235b-4bdf-89e5-80c37cc0181a', 'anthropic', 'Claude Code')
...
test('deriveChatError surfaces claude/droid model rejections with runtime-aware guidance', () => {
  ...
  const droidBadModel = deriveChatError('Invalid model: not-a-real-model', 'openai', 'Factory Droid')
