...
if (process.argv[2] === '--crash') {
  ...
  try {
    ...
    fs.symlinkSync(path.join(root, 'absent-target'), path.join(root, 'ORG', 'dangling'))
    assert.throws(() => commitWorkspaceFiles(root, [{ path: 'ORG/dangling', expectedSha256: null, content: 'bad' }]), /symbolic links/)
    assert(!fs.existsSync(path.join(root, 'absent-target')))
    fs.symlinkSync(path.join(root, 'absent-journal'), path.join(root, '.clawmax', 'template-transaction.json'))
    assert.throws(() => recoverWorkspaceFileTransaction(root), /symbolic links/)
    fs.unlinkSync(path.join(root, '.clawmax', 'template-transaction.json'))
    ...
    const binaryPath = 'SKILLS/custom/synthetic/bin/tool'
    const binaryFile = path.join(root, binaryPath)
    commitWorkspaceFiles(root, [{ path: binaryPath, expectedSha256: null, content: binary, mode: 0o700 }])
    assert.equal(sha256(fs.readFileSync(binaryFile)), sha256(binary))
    assert.equal(fs.statSync(binaryFile).mode & 0o777, 0o700, 'Skill executable mode must survive commit')
    assert.throws(() => commitWorkspaceFiles(root, [{ path: binaryPath, expectedSha256: sha256(binary), content: Buffer.alloc(binary.length, 0x41), mode: 0o700 }], () => { throw new Error('interrupted binary update') }), /interrupted binary update/)
    assert.equal(sha256(fs.readFileSync(binaryFile)), sha256(binary), 'Interrupted binary update must restore the original')
    assert.equal(fs.statSync(binaryFile).mode & 0o777, 0o700, 'Recovery must restore executable mode')
    commitWorkspaceFiles(root, [{ path: binaryPath, expectedSha256: sha256(binary), content: null }])
    assert(!fs.existsSync(binaryFile))
    const legacyPath = 'ORG/legacy-recovery.md'
    fs.writeFileSync(path.join(root, legacyPath), 'legacy-after')
    fs.writeFileSync(path.join(root, '.clawmax/template-transaction.json'), JSON.stringify({ version: 1, id: 'legacy', state: 'prepared', entries: [{ path: legacyPath, before: null, after: Buffer.from('legacy-after').toString('base64') }] }))
    recoverWorkspaceFileTransaction(root)
    assert(!fs.existsSync(path.join(root, legacyPath)), 'New recovery must settle an older v1 journal')
