...
export async function runRuntimeCli(o: {
  ...
  // Runtime credentials are injected HERE, where the target CLI is already known — deliberately
  // not via safe-env's shared allowlist. That allowlist returns one merged object for every child
  // process, so a credential added there reaches unrelated runtimes, skill installers, and helper
  // CLIs. Both agent runtimes can execute tools autonomously, making that cross-runtime exposure a
  ...
  const effectiveEnv: NodeJS.ProcessEnv = o.runtime === 'claude'
    ...
    : o.runtime === 'droid'
      ...
      // Strip both credentials for every other runtime rather than trusting callers to hand over
      // a clean env. Not all of them do: ai-generator can pass `process.env` straight through.
      : runtimeSafeEnv
