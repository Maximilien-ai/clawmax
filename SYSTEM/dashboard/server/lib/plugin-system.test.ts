...
async function run() {
  ...
  await test('generic v2 plugin validates schema, persists fields, and applies templates', () => {
    ...
    const releaseTemplates = listPluginTemplates(plugin!).filter((template) => (
      'fields' in template.payload && template.payload.fields?.release === '2.0.0-test-rc46'
    ))
    assert.strictEqual(releaseTemplates.length, 6, 'Expected the current RC46 file to expand into six independently reviewable journeys')
    ...
    assert(releaseTemplates.some((template) => template.id === '2.0.0-test-rc46:rc46-builder-ai-create-intents'), 'Expected release-qualified Builder checklist discovery')
    const applied = applyPluginTemplate(plugin!, '2.0.0-test-rc46:rc46-builder-ai-create-intents')
