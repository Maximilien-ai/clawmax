...
async function run() {
  ...
  await test('generic v2 plugin validates schema, persists fields, and applies templates', () => {
    ...
    const releaseTemplates = listPluginTemplates(plugin!).filter((template) => (
      'fields' in template.payload && template.payload.fields?.release === '2.0.0-test-rc11'
    ))
    ...
    assert(releaseTemplates.some((template) => template.id === '2.0.0-test-rc11:rc11-release-smoke'), 'Expected release-qualified checklist item discovery')
    const applied = applyPluginTemplate(plugin!, '2.0.0-test-rc11:rc11-release-smoke')
