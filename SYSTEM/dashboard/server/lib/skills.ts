...
export function getAssignedSkillPromptNotes(skillIds: string[]): string[] {
  ...
  if (skillIds.includes('clawmax-resend')) {
    guidance.push(
      ...
      '- For direct file requests like "send your soul.md file to mmaximilien@gmail.com", call `clawmax-resend-send` with `--to`, a concise `--subject`, a short `--body`, and `--attach` for the original file path.',
