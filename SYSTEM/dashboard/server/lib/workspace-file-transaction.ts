...
function read(file: string): Buffer | null {
  try {
    if (!fs.statSync(file).isFile() || fs.statSync(file).size > 2 * 1024 * 1024) throw new PortableTemplateError('resource_conflict', 'Resource is not a bounded regular file', 409)
    return fs.readFileSync(file)
  } catch (error: any) { if (error.code === 'ENOENT') return null; throw error }
}
function write(file: string, bytes: Buffer | null): void {
  if (bytes === null) {
...
export function recoverWorkspaceFileTransaction(root: string): void {
  ...
  if (journal.version !== 1 || !['prepared', 'committed'].includes(journal.state) || !Array.isArray(journal.entries) || journal.entries.length > 1024 || journal.entries.some(entry => typeof entry.path !== 'string' || (entry.before !== null && typeof entry.before !== 'string') || (entry.after !== null && typeof entry.after !== 'string'))) throw new PortableTemplateError('workspace_recovery_required', 'Invalid transaction journal', 503)
  for (const entry of journal.entries) {
    const current = encoded(read(target(root, entry.path)))
  ...
  if (journal.state === 'prepared') for (const entry of [...journal.entries].reverse()) write(target(root, entry.path), decoded(entry.before))
  fs.unlinkSync(file)
  syncDirectory(path.dirname(file))
}
...
export function commitWorkspaceFiles(rootInput: string, mutations: WorkspaceFileMutation[], afterWrite?: (index: number) => void): void {
  ...
  const entries = mutations.map(item => {
    const before = read(target(root, item.path))
    ...
    if (item.content !== null && Buffer.byteLength(item.content) > 2 * 1024 * 1024) throw new PortableTemplateError('invalid_plan', 'Resource content is oversized')
    return { path: item.path, before: encoded(before), after: item.content === null ? null : Buffer.from(item.content).toString('base64') }
  })
  const journal: Journal = { version: 1, id: crypto.randomUUID(), state: 'prepared', entries }
  ...
  locks.add(root)
  try {
    entries.forEach((entry, index) => { write(target(root, entry.path), decoded(entry.after)); afterWrite?.(index) })
