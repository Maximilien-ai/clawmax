...
test('resend partner exposes server-stored API key field and skill catalog', () => {
  ...
  assert(partner.fields?.some((field) => field.key === 'fromEmail' && field.secret !== true) === true, 'Expected resend fromEmail field')
  assert(partner.fields?.some((field) => field.key === 'fromName' && field.secret !== true) === true, 'Expected resend fromName field')
  assert(partner.fields?.some((field) => field.key === 'replyTo' && field.secret !== true) === true, 'Expected resend replyTo field')
