...
test('importAgentFromTemplate registers created agents into the active OpenClaw workspace config', async () => {
  ...
  try {
    ...
    assertEqual(registered.name, 'Briefing Writer', 'Expected imported agent display name to persist independently of its ID')
    const identity = fs.readFileSync(path.join(expectedWorkspace, 'IDENTITY.md'), 'utf-8')
    assert(identity.includes('# Briefing Writer'), 'Expected IDENTITY.md heading to preserve the template display name')
    assert(identity.includes('- **Name:** Briefing Writer'), 'Expected IDENTITY.md metadata to preserve the template display name')
