...
const invalidFixtures: Array<[string, (manifest: any) => void]> = [
  ...['id', 'slug', 'name', 'description', 'version', 'icon', 'objectKind'].map((key): [string, (manifest: any) => void] => [`missing-${key.toLowerCase()}`, (m) => { delete m[key] }]),
  ...['owner', 'repo', 'url'].map((key): [string, (manifest: any) => void] => [`source-${key}`, (m) => { m.source[key] = 42 }]),
  ['object-kind-invalid', (m) => { m.objectKind = 'Invalid Kind' }],
  ['visibility-invalid', (m) => { m.visibility = 'internal' }],
  ['source-missing', (m) => { delete m.source }],
  ['source-type-invalid', (m) => { m.source.type = 'filesystem' }],
  ['schema-null', (m) => { m.recordSchema = null }],
  ['schema-properties-missing', (m) => { delete m.recordSchema.properties }],
  ['schema-properties-string', (m) => { m.recordSchema.properties = 'invalid' }],
  ['required-nonstring', (m) => { m.recordSchema.required = [1] }],
  ['field-primitive', (m) => { m.recordSchema.properties.note = 'invalid' }],
  ['field-title-nonstring', (m) => { m.recordSchema.properties.note.title = 42 }],
  ['slider-max-missing', (m) => { m.recordSchema.properties.note = { type: 'integer', title: 'Number', control: 'slider', minimum: 0 } }],
  ['numeric-min-invalid', (m) => { m.recordSchema.properties.note = { type: 'number', title: 'Number', minimum: '0' } }],
  ['numeric-max-invalid', (m) => { m.recordSchema.properties.note = { type: 'number', title: 'Number', maximum: '1' } }],
  ['step-nonnumeric-field', (m) => { m.recordSchema.properties.note.step = 1 }],
  ['integer-default-invalid', (m) => { m.recordSchema.properties.note = { type: 'integer', title: 'Count', default: '1' } }],
  ['array-items-missing', (m) => { m.recordSchema.properties.note = { type: 'array', title: 'Tags' } }],
  ['ui-order-nonstring', (m) => { m.ui = { form: { order: [1] } } }],
