...
const currentChecklist = checklistByRelease.get('2.0.0-test-rc44')
...
assert(JSON.stringify(Array.from(checklistByRelease.keys()).sort()) === JSON.stringify(['1.9.9 regression', '2.0.0 previous RCs', '2.0.0-test-rc44']), 'Review set names must be stable and unambiguous')
assert(currentChecklist?.items.length === 6, 'Current RC44 must contain only the six checks requiring independent review')
...
assert(currentChecklist?.items.some((item: any) => item.id === 'rc44-builder-create-and-chat'), 'Current RC must independently review the repaired Builder creation journey')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc44-partner-and-skill-boundaries'), 'Current RC must independently review partner and skill clarity')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc44-lmstudio-gemma-chat'), 'Current RC must independently run the reported LM Studio model')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc44-qbo-runtime-install'), 'Current RC must independently run QBO on both image architectures')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc44-resource-retention-soak'), 'Current RC must retain the external resource-soak launch gate')
...
assert(pageSource.includes("return ['1.9.9', '2.0.0', '2.0.0-test-rc44'].filter"), 'Review filters must stay limited to the three release families')
