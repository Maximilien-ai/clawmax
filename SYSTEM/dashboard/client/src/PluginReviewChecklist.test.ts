...
assert(currentChecklist?.items.length === 7, 'Current RC37 must contain only the seven checks requiring independent review')
...
assert(currentChecklist?.items.some((item: any) => item.id === 'rc37-ai-create-routing-journey'), 'Current RC must independently review explicit Builder AI Create routing')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc37-ai-editor-expand-journey'), 'Current RC must independently review AI Editor output quality')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc37-lifecycle-real-data-xray'), 'Current RC must independently review real-data Lifecycle X-rays')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc37-activity-sharing-consent-journey'), 'Current RC must independently review activity-sharing consent comprehension')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc37-gmail-oauth-lifecycle'), 'Current RC must exercise the real Gmail provider')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc37-microsoft-oauth-lifecycle'), 'Current RC must exercise the real Microsoft provider')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc37-persistent-session-recovery'), 'Current RC must exercise recovery in a persistent upgraded runtime')
