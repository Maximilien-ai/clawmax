...
const currentChecklist = checklistByRelease.get('2.0.0-test-rc21')
...
assert(JSON.stringify(Array.from(checklistByRelease.keys()).sort()) === JSON.stringify(['1.9.9 regression', '2.0.0 previous RCs', '2.0.0-test-rc21']), 'Review set names must be stable and unambiguous')
assert(currentChecklist?.items.length === 10, 'Current RC21 must contain ten focused plugin and release checks')
assert(previousChecklist?.items.length === 48, 'Unconfirmed earlier 2.0 checks, including RC18, must remain in the cumulative set')
...
assert(currentChecklist?.items.some((item: any) => item.id === 'rc21-guardrail-complete-flow'), 'Current RC must verify the complete Guardrail flow')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc21-eval-complete-flow'), 'Current RC must verify the complete Eval flow')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc21-optimize-complete-flow'), 'Current RC must verify the complete Optimize flow')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc21-authorized-suggestion-catalogs'), 'Current RC must verify every authorized suggestion catalog')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc21-release-health-and-persistence'), 'Current RC must verify release health, persistence, and Review export')
...
assert(pageSource.includes("return ['1.9.9', '2.0.0', '2.0.0-test-rc21'].filter"), 'Review filters must stay limited to the three release families')
