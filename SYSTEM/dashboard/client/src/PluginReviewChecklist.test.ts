...
const currentChecklist = checklistByRelease.get('2.0.0-test-rc22')
...
assert(JSON.stringify(Array.from(checklistByRelease.keys()).sort()) === JSON.stringify(['1.9.9 regression', '2.0.0 previous RCs', '2.0.0-test-rc22']), 'Review set names must be stable and unambiguous')
assert(currentChecklist?.items.length === 6, 'Current RC22 must contain six focused plugin and release checks')
assert(previousChecklist?.items.length === 58, 'Unconfirmed earlier 2.0 checks through RC21 must remain in the cumulative set')
...
assert(currentChecklist?.items.some((item: any) => item.id === 'rc22-release-identity-and-catalogs'), 'Current RC must verify every packaged suggestion catalog')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc22-plugin-default-order-and-memory'), 'Current RC must verify plugin ordering and browser memory')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc22-enabled-disabled-consistency'), 'Current RC must verify lifecycle state across views')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc22-eval-run-controls-and-case-progress'), 'Current RC must verify Eval run controls and case progress')
...
assert(pageSource.includes("return ['1.9.9', '2.0.0', '2.0.0-test-rc22'].filter"), 'Review filters must stay limited to the three release families')
