...
const currentChecklist = checklistByRelease.get('2.0.0-test-rc37')
...
assert(JSON.stringify(Array.from(checklistByRelease.keys()).sort()) === JSON.stringify(['1.9.9 regression', '2.0.0 previous RCs', '2.0.0-test-rc37']), 'Review set names must be stable and unambiguous')
assert(currentChecklist?.items.length === 20, 'Current RC37 must retain outstanding checks and add session-conflict recovery coverage')
assert(previousChecklist?.items.length === 72, 'Unconfirmed earlier 2.0 checks through RC24 must remain in the cumulative set')
...
assert(currentChecklist?.items.some((item: any) => item.id === 'rc25-lifecycle-suggested-relationship'), 'Current RC must verify the populated Lifecycle Suggested graph')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc25-lifecycle-suggested-list-no-xray'), 'Current RC must verify Suggested list behavior')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc25-lifecycle-communication-xray'), 'Current RC must verify group and community X-rays')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc25-lifecycle-mobile-graph'), 'Current RC must verify Lifecycle mobile graph behavior')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc35-builder-explicit-create-handoffs'), 'Current RC must verify explicit Builder AI Create handoffs')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc35-ai-editor-expand-surfaces'), 'Current RC must verify the shared AI editor and Expand with AI')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc35-lifecycle-targets-loading-xray'), 'Current RC must verify target-specific Lifecycle X-ray loading')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc35-plugin-target-summary'), 'Current RC must verify target summaries for active plugin records')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc35-activity-sharing-pill-revoke'), 'Current RC must verify activity-sharing pill revocation')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc35-partner-sharing-revoke'), 'Current RC must verify per-destination Partner revocation')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc37-runtime-identity-and-restart'), 'Current RC must verify RC37 identity and restart persistence')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc37-openclaw-runtime-and-skills'), 'Current RC must verify the OpenClaw runtime and plugin-owned skill catalog')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc37-openclaw-session-recovery'), 'Current RC must verify automatic chat and workflow session recovery')
...
assert(pageSource.includes("return ['1.9.9', '2.0.0', '2.0.0-test-rc37'].filter"), 'Review filters must stay limited to the three release families')

console.log('PluginReviewChecklist.test.ts: 93 tests passed')
