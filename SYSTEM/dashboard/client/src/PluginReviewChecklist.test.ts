...
const currentChecklist = checklistByRelease.get('2.0.0-test-rc46')
...
assert(JSON.stringify(Array.from(checklistByRelease.keys()).sort()) === JSON.stringify(['1.9.9 regression', '2.0.0 previous RCs', '2.0.0-test-rc46']), 'Review set names must be stable and unambiguous')
assert(currentChecklist?.items.length === 6, 'Current RC46 must contain only the six checks requiring independent review')
...
assert(currentChecklist?.items.some((item: any) => item.id === 'rc46-builder-ai-create-intents'), 'Current RC must independently review every guaranteed AI creation path')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc46-claude-code-runtime'), 'Current RC must independently run the Claude Code runtime')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc46-factory-droid-runtime'), 'Current RC must independently run the Factory Droid runtime')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc46-lmstudio-gemma-chat'), 'Current RC must independently run the reported LM Studio model')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc46-qbo-runtime-install'), 'Current RC must independently run QBO on both image architectures')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc46-resource-retention-soak'), 'Current RC must retain the external resource-soak launch gate')
...
assert(pageSource.includes("return ['1.9.9', '2.0.0', '2.0.0-test-rc46'].filter"), 'Review filters must stay limited to the three release families')
