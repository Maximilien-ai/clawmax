...
const currentChecklist = checklistByRelease.get('2.0.0-test-rc38')
...
assert(JSON.stringify(Array.from(checklistByRelease.keys()).sort()) === JSON.stringify(['1.9.9 regression', '2.0.0 previous RCs', '2.0.0-test-rc38']), 'Review set names must be stable and unambiguous')
assert(currentChecklist?.items.length === 7, 'Current RC38 must contain only the seven checks requiring independent review')
...
assert(currentChecklist?.items.some((item: any) => item.id === 'rc38-ai-create-routing-journey'), 'Current RC must independently review explicit Builder AI Create routing')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc38-ai-editor-expand-journey'), 'Current RC must independently review AI Editor output quality')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc38-lifecycle-real-data-xray'), 'Current RC must independently review real-data Lifecycle X-rays')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc38-activity-sharing-consent-journey'), 'Current RC must independently review activity-sharing consent comprehension')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc38-gmail-oauth-lifecycle'), 'Current RC must exercise the real Gmail provider')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc38-microsoft-oauth-lifecycle'), 'Current RC must exercise the real Microsoft provider')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc38-persistent-session-recovery'), 'Current RC must exercise recovery in a persistent upgraded runtime')
...
assert(pageSource.includes("return ['1.9.9', '2.0.0', '2.0.0-test-rc38'].filter"), 'Review filters must stay limited to the three release families')
