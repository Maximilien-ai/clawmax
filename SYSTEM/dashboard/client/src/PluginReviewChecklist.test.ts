...
const currentChecklist = checklistByRelease.get('2.0.0-test-rc39')
...
assert(JSON.stringify(Array.from(checklistByRelease.keys()).sort()) === JSON.stringify(['1.9.9 regression', '2.0.0 previous RCs', '2.0.0-test-rc39']), 'Review set names must be stable and unambiguous')
assert(currentChecklist?.items.length === 3, 'Current RC39 must contain only the three checks requiring independent review')
...
assert(currentChecklist?.items.some((item: any) => item.id === 'rc39-agent-discovery-and-details'), 'Current RC must independently review agent discovery and partial-data resilience')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc39-chat-stream-resilience'), 'Current RC must independently review long-running chat resilience')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc39-workflow-lifecycle-controls'), 'Current RC must independently review workflow lifecycle controls')
...
assert(pageSource.includes("return ['1.9.9', '2.0.0', '2.0.0-test-rc39'].filter"), 'Review filters must stay limited to the three release families')
