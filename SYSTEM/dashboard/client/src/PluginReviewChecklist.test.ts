...
const currentChecklist = checklistByRelease.get('2.0.0-test-rc23')
...
assert(JSON.stringify(Array.from(checklistByRelease.keys()).sort()) === JSON.stringify(['1.9.9 regression', '2.0.0 previous RCs', '2.0.0-test-rc23']), 'Review set names must be stable and unambiguous')
assert(currentChecklist?.items.length === 3, 'Current RC23 must contain three focused plugin deployment checks')
assert(previousChecklist?.items.length === 64, 'Unconfirmed earlier 2.0 checks through RC22 must remain in the cumulative set')
...
assert(currentChecklist?.items.some((item: any) => item.id === 'rc23-private-catalog-upgrade'), 'Current RC must verify private catalogs replace fallback catalogs')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc23-plugin-order-after-upgrade'), 'Current RC must verify plugin ordering after inventory migration')
assert(currentChecklist?.items.some((item: any) => item.id === 'rc23-optimize-legend-stability'), 'Current RC must verify the Optimize graph legend remains stable')
...
assert(pageSource.includes("return ['1.9.9', '2.0.0', '2.0.0-test-rc23'].filter"), 'Review filters must stay limited to the three release families')
