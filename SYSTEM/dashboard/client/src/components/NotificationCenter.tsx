...
export function NotificationCenter({ onNavigateToAgent, onNavigateToWorkflow, onNavigateToPage, onNavigateToDoc, onAgentRestarted }: NotificationCenterProps) {
  ...
  const handleDismiss = async (id: string) => {
    ...
    await fetch('/api/notifications/dismiss', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, groupedIds: notification?.groupedIds }),
    })
    fetchNotifications()
  }
                    ...
                    >
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {n.grouped && n.groupedChildren && n.groupedChildren.length > 0 && (
                            <div className="mt-2 space-y-1.5 rounded-lg border border-gray-100 bg-gray-50/80 px-2.5 py-2 dark:border-gray-700 dark:bg-gray-900/30">
                              {n.groupedChildren.slice(0, 6).map(child => (
                                <div key={child.id} className="flex items-start justify-between gap-3 text-[11px]">
                                  ...
                                  {child.type === 'artifact-update' && (child.artifactPath || child.artifactUrl) && (
                                    <button
                                      ...
                                      className="shrink-0 text-sky-600 hover:text-sky-700 dark:text-sky-400 dark:hover:text-sky-300 hover:underline"
                                    >
                                      Open
                                    </button>
                                  )}
                              ...
                              {n.groupedChildren.length > 6 && (
                                <div className="text-[11px] text-gray-500 dark:text-gray-400">
                                  +{n.groupedChildren.length - 6} more similar notifications
                                </div>
                              )}
                            </div>
                          )}
