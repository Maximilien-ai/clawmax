...
- Gmail and Microsoft 365 now have public preview partner definitions, bounded list/search/read/draft capabilities, encrypted OAuth persistence, PKCE/replay protection, refresh/disconnect, and production Google/Microsoft identity adapters behind operator configuration. Provider scopes are mapped from a fixed ClawMax capability allowlist and never include mail send. Mailbox adapters and connection UI remain pending test-account and container validation.
