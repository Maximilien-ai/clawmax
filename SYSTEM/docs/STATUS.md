...
**Status**: `main` is being prepared for `1.8.2-test-rc1` after the first simplify/optimize slice landed on top of the earlier `1.8.x` hardening work.
...
- The latest reported full local integration run before the current follow-up fixes was `321/321` passing with `--with-validation`.
...
- Near-term target: build and validate `1.8.2-test-rc1`, then continue the `1.8.x` optimize/harden pass if cloud/on-prem checks stay green.
