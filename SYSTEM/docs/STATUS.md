...
- The 2.0 source security review is complete at reviewed code `79262f35`: no Critical findings, all eight High findings fixed, one Medium fixed, and three Medium deployment/supply-chain risks formally accepted through September 30 follow-ups. The final RC38 report appendix still needs exact image digests and final cloud/on-prem/runtime evidence.
...
- Post-RC37 RC38 hardening now enforces credentialed CORS allowlists, cloud bypass rejection, complete route authorization, random-token shared dashboards, bounded ZIP extraction, argument-vector subprocesses, response security headers, and an exception-free zero-vulnerability dependency gate. RC38 has not been cut; the final full suite and image gates must run after the documentation reconciliation.
