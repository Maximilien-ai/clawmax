...
- [ ] **1.9.9 brokered agent-skill secrets** — let explicitly authorized skills use named workspace/runtime secrets without exposing a general vault reader or injecting raw values into the parent agent process. Includes grant/fingerprint checks, masked readiness, redaction, controlled Google credential guidance, and local/container security-negative tests. Plan: [SIMPLIFY_HARDEN_OPTIMIZE_1_9_9.md](planning/SIMPLIFY_HARDEN_OPTIMIZE_1_9_9.md).
- [ ] **1.9.9 native xAI/Grok provider** — add first-class xAI key/readiness, native `xai/...` model discovery/selection, actionable failures, and chat/workflow/container tests. Advertise Grok 4.5 only after the pinned OpenClaw image executes that exact model; isolate any required runtime upgrade from the RC. Plan: [PUBLIC_MODELS_GATEWAYS_EMAIL_1_9_9_2_0.md](planning/PUBLIC_MODELS_GATEWAYS_EMAIL_1_9_9_2_0.md).
- [ ] **1.9.9 native OpenRouter gateway** — add OpenRouter as a hosted provider using native `openrouter/...` identities, key/readiness, discovery, and provider-correct execution. Do not route it through the LM Studio-oriented generic OpenAI-compatible path. Plan: [PUBLIC_MODELS_GATEWAYS_EMAIL_1_9_9_2_0.md](planning/PUBLIC_MODELS_GATEWAYS_EMAIL_1_9_9_2_0.md).
...
- [ ] **2.0 public plugin and AI-scoring foundation** — publish generic plugin/scorer contracts, a shared explainable rubric, prompt and generated-artifact score shapes, and the first user-facing scoring flows. Proprietary guardrail and evaluation plugin implementations remain private and must not be bundled in the public repository or default image.
...
- [ ] **Secret readiness follow-through** — as part of `1.9.9`, distinguish browser-local capture from secure agent-runtime availability and report missing/present/degraded/authorization-required states before assignment, apply, chat, or workflow run.
