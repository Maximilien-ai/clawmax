...
- [ ] **Validate `1.7.0` built images with upgraded OpenClaw on cloud and on-prem** — automated tests and local upgrade smokes are green, but the first `1.7.0` image line still needs real built-artifact validation for agent chat, template apply, workflow execution, Skills setup, `/api/system`, and durable gateway startup across cloud and on-prem runtimes.
...
- [ ] **Verify `1.7.0` image packaging with upgraded OpenClaw** — the newer packaged OpenClaw path is now wired into `main`, but keep a focused follow-through item until the first cloud/on-prem `1.7.0` images are validated live with `openclaw --version`, real chat/workflow execution, and healthy packaged skills/channels.
...
- [ ] **Keep OpenClaw currency current after `1.7.0`** — add a short recurring release-engineering pass to check the tested OpenClaw target against upstream every couple of weeks, update the pin deliberately, and rerun chat/workflow/skills/gateway smoke before each upgrade cut.
