...
The release-blocking subset and current RC46 order are maintained in
...
- [ ] **RC46 bounded-resource gate** — issue [#187](https://github.com/Maximilien-ai/clawmax/issues/187) remains a stable-release blocker. The retained-descendant lifecycle defect and repeatable Podman collector are fixed on `main`; complete a controlled 24-hour combined-image soak with baseline, interval, peak/final RSS/PID, process inventory, architecture, digest, and workload evidence before promotion.
- [ ] **2.0 security release-artifact appendix** — the source audit, threat model, 29-family authorization matrix, dynamic boundary tests, findings register, SBOM/license inventory, dependency/secret scans, Medium-risk decisions, and RC43 historical image evidence are recorded. There are no unresolved Critical/High findings. Before promotion, replace the final-candidate artifact references with accepted RC46-or-later public and combined-image evidence, then append cloud/on-prem runtime evidence and the Review approver to [SECURITY_REVIEW_2_0_RC38.md](security/SECURITY_REVIEW_2_0_RC38.md).
...
- [ ] **External plugin compatibility** — keep deployment-managed implementations, catalogs, and product-specific tests in their owning repositories. The public host may provide only generic adapters, schemas, and synthetic contract fixtures.
...
### UX & Product Polish
- [ ] **Authenticated extension image selection** — update the cloud/on-prem control plane to accept an authorized separately distributed image override, attach the required registry pull secret, and report the selected image and pull remediation without silently substituting synthetic public fixtures.
...
- [ ] **Plugin relationship graph navigation parity** — validate shared zoom controls for contributed relationship views at desktop/mobile sizes and add pointer/touch panning if tester feedback shows native scrolling is insufficient.
