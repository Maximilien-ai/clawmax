...
- [ ] **Public Gmail and Microsoft 365 mail partners** — the shared versioned read/search/draft schema, exact grant boundary, fake provider, prompt-injection tests, and honest preview partner entries are implemented. Next: delegated OAuth/PKCE connected-account storage, Gmail API and Microsoft Graph adapters, then test-account/container validation. Send and destructive actions remain separate and disabled. Never request normal mailbox passwords. Plan: [PUBLIC_MODELS_GATEWAYS_EMAIL_2_0.md](planning/PUBLIC_MODELS_GATEWAYS_EMAIL_2_0.md).
