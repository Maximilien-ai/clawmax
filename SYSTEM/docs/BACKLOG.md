...
- [ ] **Release-blocking 2.0 dashboard security audit** — before promoting `2.0.0`, complete a documented review of authentication/session handling, authorization and workspace isolation, secret storage and runtime grants, plugin trust boundaries, API/input validation, uploads and filesystem access, agent/runtime command execution, outbound requests and activity export, logs/redaction, container defaults, dependencies, and CI/supply-chain packaging. Record reproducible findings with severity, owner, remediation, and regression evidence. Release requires no unresolved Critical or High findings; any accepted Medium risk needs an explicit owner and follow-up date. Plan: [SECURITY_AUDIT_2_0.md](planning/SECURITY_AUDIT_2_0.md).
- [ ] **Public Gmail and Microsoft 365 mail partners** — the shared capability/grant boundary, malicious-message tests, encrypted OAuth lifecycle, production identity adapters, Partner connect/manage UI, and Gmail/Graph list/search/read/draft adapters are implemented. Raw provider scopes and header injection fail closed; no send operation exists, Graph excludes `Mail.Send`, and Gmail's provider-level compose scope is contained behind a draft-only host adapter. Next: persisted agent/plugin mail grants, short-lived runtime execution, test-account OAuth/mailbox validation, then container validation. Send and destructive actions remain separate and disabled. Never request normal mailbox passwords. Plan: [PUBLIC_MODELS_GATEWAYS_EMAIL_2_0.md](planning/PUBLIC_MODELS_GATEWAYS_EMAIL_2_0.md).
- [ ] **Public consent-gated Activity Export and Digo partner** — add a generic per-user consent receipt, scoped canonical activity events, secret redaction, durable nonblocking outbox, visible sharing state, immediate opt-out, and authenticated/idempotent destination adapters. Prove the contract against a dedicated ClawMax.ai reference receiver before enabling Digo, and require new consent when the named destination changes. Plan: [PUBLIC_ACTIVITY_EXPORT_PARTNERS_2_0.md](planning/PUBLIC_ACTIVITY_EXPORT_PARTNERS_2_0.md).
...
- [ ] **Public Lifecycle plugin** — turn the initial read-only skeleton into one agent/workflow inspection surface for identity, configuration, activity, artifacts, outputs, and history. Keep V1 diagnostic-only: it must not silently edit configuration, run work, or expose secrets. Plan: [PUBLIC_LIFECYCLE_PLUGIN_2_0.md](planning/PUBLIC_LIFECYCLE_PLUGIN_2_0.md).
- [ ] **Plugin manager** — add a small settings action to the Plugins section that lists discovered plugins with availability, compatibility, health, and an explicit enable/disable checkbox. Persist operator-approved state at the correct scope, require confirmation for disabling a running plugin, preserve plugin data by default, and explain when a restart is required. Dev should discover all local public and private plugins when the sibling private repository is available; the public image must still enable only public defaults.
...
- [ ] **Keep OpenClaw currency current** — check the tested `v2026.6.11` target against upstream every couple of weeks, update the pin deliberately, and rerun chat/workflow/skills/gateway smoke before each upgrade cut.
