...
- [ ] **Plugin architecture MVP1 follow-through** — the first generic declarative v2 host contract, operator health diagnostics, and deny-by-default enforcement for document, notification, and workspace-context capabilities are merged. Continue with manifest-declared custom actions and action-specific grants, external packaging validation, plugin page/data loading speed, DocHub/file-open polish, and shared UI parity with Agents/Workflows.
