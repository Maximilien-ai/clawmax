...
- [ ] **1.8.x hardening + simplification follow-through** — `1.8.0` through `1.8.7` now cover the first stability, responsiveness, Add Agent, delete-flow, DocHub, notification, workflow session, and model lifecycle cleanup slices. Continue from [HARDENING_SIMPLIFICATION_1_8_X.md](/Users/maximilien/github/Maximilien-ai/clawmax-codex/SYSTEM/docs/planning/HARDENING_SIMPLIFICATION_1_8_X.md) plus the active `1.8.6` / `1.8.7` planning notes and the remaining follow-through below.
- [ ] **Cloud/on-prem `1.8.7` release validation** — validate `1.8.7-test-rc6` on real cloud and on-prem deployments, focusing on workflow restart/retry behavior, agent action menus, remaining DocHub opens, and safe deprecated-model handling for OpenAI, Anthropic, and Gemini.
