...
- [ ] **1.8.x hardening + simplification follow-through** — the first `1.8.0` pass is merged to `main`; continue from [HARDENING_SIMPLIFICATION_1_8_X.md](/Users/maximilien/github/Maximilien-ai/clawmax-codex/SYSTEM/docs/planning/HARDENING_SIMPLIFICATION_1_8_X.md) and treat remaining items as `1.8.0` promotion checks or `1.8.1+` follow-through.
- [ ] **Cloud/on-prem `1.8.0` release validation** — validate `1.8.0-test-rc4` on real cloud and on-prem deployments before promotion, focusing on workflow execution auth fallback, mobile layout fixes, notifications/doc links, Doctor state, Resend/Cognee partner/runtime paths, and clean workspace/test isolation behavior.
