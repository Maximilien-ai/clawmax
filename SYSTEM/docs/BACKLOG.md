...
- [ ] **2.0 public Optimize plugin** — the public plugin now includes eight agent/workflow/workspace planning suggestions, automatic model-selection recommendations, and bounded token, cost, duration, and quality controls. Next: workflow-first measured accounting, budget-period allocations, per-run forecasts, sourced pricing, confirmed schedule/model application, history, and reversible application. Tokens remain authoritative when model pricing is unavailable; unknown pricing is never treated as zero. Plan: [PUBLIC_OPTIMIZE_PLUGIN_2_0.md](planning/PUBLIC_OPTIMIZE_PLUGIN_2_0.md).
