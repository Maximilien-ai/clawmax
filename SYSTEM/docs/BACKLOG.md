...
- [ ] **1.8.x hardening + simplification follow-through** — `1.8.0` through `1.8.5` are landing the first stability, responsiveness, Add Agent, delete-flow, DocHub, and notification cleanup slices. Continue from [HARDENING_SIMPLIFICATION_1_8_X.md](/Users/maximilien/github/Maximilien-ai/clawmax-codex/SYSTEM/docs/planning/HARDENING_SIMPLIFICATION_1_8_X.md) and the remaining post-`1.8.5` follow-through below.
- [ ] **Cloud/on-prem `1.8.5` release validation** — validate `1.8.5-test-rc1` on real cloud and on-prem deployments, focusing on DocHub file opens/previews, notification dedupe/grouping, and clearer provider cooldown/auth messaging.
...
- [ ] **Provider cooldown/auth surfacing follow-through** — the first operator-facing message cleanup shipped in `1.8.5`; keep refining workflow/result/log UX so users can tell transient timeout/cooldown windows from hard auth or quota/config failures without reading raw fallback chains.
