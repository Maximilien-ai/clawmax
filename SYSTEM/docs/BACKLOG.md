...
- [ ] **1.9.3 chat archives + validation-regression track** — `1.9.2` is now promoted. Continue the next release around `#158` and `#159`: broken archive list metadata/timestamps/title parsing plus archived conversation restore/resume, and fold in the newly reported Templates mobile overflow plus Workflows first-load stall from `1.9.2` cloud validation. Use [SIMPLIFY_HARDEN_OPTIMIZE_1_9_3.md](/Users/maximilien/github/Maximilien-ai/clawmax-codex/SYSTEM/docs/planning/SIMPLIFY_HARDEN_OPTIMIZE_1_9_3.md) as the focused scope.
...
- [ ] **OpenClaw update track (after 1.9.4 unless forced sooner)** — once `1.9.4` stabilizes the provider/runtime incident surfaces, take the planned OpenClaw update with the stronger wrapper/coverage baseline already established in `1.9.1` and extended in `1.9.2`/`1.9.3`.
