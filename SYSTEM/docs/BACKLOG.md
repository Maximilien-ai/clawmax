...
- [ ] **Keep OpenClaw currency current after `1.7.3`** — add a short recurring release-engineering pass to check the tested OpenClaw target against upstream every couple of weeks, update the pin deliberately, and rerun chat/workflow/skills/gateway smoke before each upgrade cut.
