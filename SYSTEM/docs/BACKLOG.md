...
- [ ] **System test communications prompt still causes false channel-send failures** — the ClawMax System Test communications workflow wording encourages some agents to think they must self-send through an external channel/plugin instead of returning content for ClawMax to post. Template/workflow wording is being tightened, but rerun validation is still needed.
