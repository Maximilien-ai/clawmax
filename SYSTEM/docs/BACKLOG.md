...
- [ ] **Validate `1.7.1` built images with upgraded OpenClaw on cloud and on-prem** — local/manual and automated release-candidate validation is green, but the first `1.7.1` built-artifact line still needs real cloud/on-prem image validation for agent chat, template apply, workflow execution, Skills setup sessions, `/api/system`, and durable gateway startup.
...
- [ ] **Verify `1.7.1` image packaging with upgraded OpenClaw** — the newer packaged OpenClaw path is now wired into `main`, but keep a focused follow-through item until the first cloud/on-prem `1.7.1` images are validated live with `openclaw --version`, real chat/workflow execution, LM Studio/Ollama follow-through, and healthy packaged skills/channels.
...
- [ ] **Keep OpenClaw currency current after `1.7.1`** — add a short recurring release-engineering pass to check the tested OpenClaw target against upstream every couple of weeks, update the pin deliberately, and rerun chat/workflow/skills/gateway smoke before each upgrade cut.
