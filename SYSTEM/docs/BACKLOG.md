...
- [ ] **Public Gmail and Microsoft 365 mail partners** — the shared read/search/draft schema, exact grant boundary, malicious-message tests, OAuth state/PKCE lifecycle, encrypted connected-account persistence, refresh/disconnect, fake exchanges, and production Google/Microsoft identity adapters are implemented. Raw provider scopes fail closed; no mail-send permission is requested. Next: partner connection UI, test-account OAuth validation, Gmail API and Microsoft Graph mailbox adapters, then container validation. Send and destructive actions remain separate and disabled. Never request normal mailbox passwords. Plan: [PUBLIC_MODELS_GATEWAYS_EMAIL_2_0.md](planning/PUBLIC_MODELS_GATEWAYS_EMAIL_2_0.md).
