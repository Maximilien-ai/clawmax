...
- [ ] **OpenClaw upgrade readiness follow-through** — the June 1 audit is complete and archived under `SYSTEM/docs/planning/archive/OPENCLAW_UPGRADE_READINESS_AUDIT_2026-06-01.md`. The focused `/api/system` route test, explicit `install/setup/update/uninstall` coverage, broad route-contract coverage across all current server routes, and the first OpenClaw/Gateway contract suite are now in place. Before the `1.6.x` line ends, keep the remaining risk narrow: page-level/browser smoke for Skills/Templates apply/Agents create plus any extra runtime contract assertions we discover while rehearsing the upgrade branch.
