...
- [ ] **System-test workspace isolation hardening** — make integration workspace setup/teardown deterministic and prove system-test organizations, groups, communities, workflows, and agents do not leak into the personal/default workspace after repeated runs.
- [ ] **Gateway recovery and probe-noise hardening** — add Doctor-driven gateway recovery for unhealthy gateway states and reduce benign dashboard probe/handshake noise in normal logs.
- [ ] **Runtime partner regression safety** — keep Resend and Cognee covered across dev, cloud, and on-prem image paths, including runtime-injected secrets, agent tool execution, plugin install/uninstall, and benign plugin warning filtering.
