...
- [ ] **RC38 security release-artifact appendix** — the source audit, threat model, 29-family authorization matrix, dynamic boundary tests, findings register, SBOM/license inventory, dependency/secret scans, and Medium-risk decisions are complete. There are no unresolved Critical/High findings. Before promotion, append the final public amd64/arm64 digests, private combined-image acceptance reference, cloud/on-prem runtime evidence, final full-suite result, and Review approver to [SECURITY_REVIEW_2_0_RC38.md](security/SECURITY_REVIEW_2_0_RC38.md).
