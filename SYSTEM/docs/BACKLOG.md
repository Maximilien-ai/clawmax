...
- [ ] **1.9.4 provider/runtime incident-hardening track** — after `1.9.3` promotion, focus the next release on shared provider-auth incident detection, repeated cron/workflow failure dedupe, and stronger doctor/preflight diagnostics for cloud/on-prem runtime provider configuration. Use [SIMPLIFY_HARDEN_OPTIMIZE_1_9_4.md](/Users/maximilien/github/Maximilien-ai/clawmax-codex/SYSTEM/docs/planning/SIMPLIFY_HARDEN_OPTIMIZE_1_9_4.md) as the active scope.
- [ ] **1.9.3 follow-through only if RC2/customer validation finds a ship blocker** — otherwise treat `1.9.3` as promotable and keep any archive/mobile/workflow cleanup out of the `1.9.4` line. Historical sprint scope is archived under [planning/archive/SIMPLIFY_HARDEN_OPTIMIZE_1_9_3.md](/Users/maximilien/github/Maximilien-ai/clawmax-codex/SYSTEM/docs/planning/archive/SIMPLIFY_HARDEN_OPTIMIZE_1_9_3.md).
- [ ] **1.9.5 performance-baseline track** — after `1.9.4`, add lightweight timed measurements for core user flows like direct agent chat latency, workflow first-load latency, and other basic dashboard/runtime actions so we can compare whether releases are getting slower or faster over time.
- [ ] **1.9.5 performance-baseline track** — after `1.9.4`, add lightweight timed measurements for core user flows like direct agent chat latency, workflow first-load latency, and other basic dashboard/runtime actions so we can compare whether releases are getting slower or faster over time. Active plan: [SIMPLIFY_HARDEN_OPTIMIZE_1_9_5.md](/Users/maximilien/github/Maximilien-ai/clawmax-codex/SYSTEM/docs/planning/SIMPLIFY_HARDEN_OPTIMIZE_1_9_5.md).
- [ ] **OpenClaw update track (`1.9.6` unless forced sooner)** — once `1.9.5` gives us a usable performance baseline, take the planned OpenClaw update and use the before/after measurements to judge whether the newer runtime improves or regresses chat/workflow responsiveness.
- [ ] **Builder output consistency + generated handoff correctness (`1.9.6`)** — investigate the demo-reported builder inconsistency where the same prompt (`AI-Driven Inbound Sales & Support Management`) produced materially different team/company outputs and at least one broken handoff. When a prompt is ambiguous enough to support either shape, prefer offering both team and company options so the user can choose explicitly. For generated company/team outputs, verify that upstream workflows actually complete and hand off so downstream workflows start reliably, and add a visual handoff representation so stalled downstream workflows are inspectable instead of hidden. Keep this tied to the real generated artifacts from the `1.9.x` workspace. Use [SIMPLIFY_HARDEN_OPTIMIZE_1_9_6.md](/Users/maximilien/github/Maximilien-ai/clawmax-codex/SYSTEM/docs/planning/SIMPLIFY_HARDEN_OPTIMIZE_1_9_6.md).
...
- [ ] **Keep OpenClaw currency current after `1.7.9`** — add a short recurring release-engineering pass to check the tested OpenClaw target against upstream every couple of weeks, update the pin deliberately, and rerun chat/workflow/skills/gateway smoke before each upgrade cut.
