...
- [ ] **Cloud deployment initial boot/runtime may be blocked by gateway misconfiguration or health** — live cloud deployments are not coming up cleanly, and the current leading suspicion is the same gateway/runtime reliability seam affecting local execution. Need a focused deployment audit covering gateway availability, Doctor parity, auth/env defaults, and startup assumptions before release.
- [ ] **Cloud agents still return gateway fixture responses** — on cloud instances, agent chat can still return repeated `openclaw fixture` output instead of real execution. Treat as a cloud runtime/bootstrap blocker until deployed agents run actual OpenClaw sessions.
- [ ] **Cloud Skills page can be empty even when workspace has agents** — cloud instances can show no skills despite active agents being present, which suggests runtime registration/bootstrap drift between the workspace UI and skill discovery paths.
