...
- [ ] **Workflow targeting conflates participants with output channels** — workflows that target a group for posting also execute every agent in that group, which makes kickoff/lead-owned workflows run the whole team when the template only intends one owner plus group-visible output.
- [ ] **Workflow participant fan-out is serial, not parallel** — agents inside a single workflow execution currently run one-by-one, so a slow or timed-out participant blocks later participants and makes progress/status misleading for supposedly parallel team work.
...
- [ ] **Workflow execution does not reliably respect selected model/provider and BYOK precedence** — users can select a local model such as `ollama/qwen...`, but execution still appears to run on Anthropic/OpenAI BYOK. Needs end-to-end audit of model resolution, provider selection, runtime auth patching, gateway/local fallback, and BYOK precedence during chat/workflow execution.
