...
- [ ] **Cloud GitHub integration should not depend only on `gh` CLI readiness** — current GitHub readiness/auth relies on the runtime having a working `gh` CLI session, which is too brittle for cloud deployments. Add a cloud-safe GitHub connection path based on API token or GitHub App/OAuth context, keep `gh` as a local/dev-friendly path, and make readiness messaging reflect which mode is active.
