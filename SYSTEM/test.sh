...
if grep -q "PluginReviewChecklist.test.ts: 30 tests passed" /tmp/clawmax-plugin-review-checklist.out; then
  pass "Plugin release checklist regression tests (30 tests)"
else
...
if grep -q "PluginWorkspaceLayout.test.ts: 29 tests passed" /tmp/clawmax-plugin-workspace-layout.out; then
  pass "Plugin workspace layout regression tests (29 tests)"
else
