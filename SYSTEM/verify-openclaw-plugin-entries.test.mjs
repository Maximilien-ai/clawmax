...
try {
  ...
  assert.throws(() => verifyPluginEntries(root), /missing plugin manifest/)
  fs.writeFileSync(path.join(plugin, 'openclaw.plugin.json'), '{}')
  ...
  verifyPluginEntries(root)
  fs.unlinkSync(path.join(plugin, 'openclaw.plugin.json'))
  assert.throws(() => verifyPluginEntries(root), /missing plugin manifest/)
