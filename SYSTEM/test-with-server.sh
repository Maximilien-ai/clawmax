...
if [ "$RUN_INTEGRATION" = true ] && [ "${CLAWMAX_TEST_ISOLATED_GATEWAY:-}" != true ]; then
  exec node "$SCRIPT_DIR/test-isolated-gateway.mjs" "$@"
fi
...
ensure_target_openclaw_for_integration() {
  ...
  if [ "${CLAWMAX_TEST_ISOLATED_GATEWAY:-}" = true ]; then
    # Parent owns and authenticated the foreground gateway, not a LaunchAgent.
    return 0
  fi

  echo "Integration tests require the isolated gateway supervisor" >&2
  return 1
}
