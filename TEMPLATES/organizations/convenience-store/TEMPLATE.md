...
agents:
  - id: store-manager
    name: Store Manager
    role: >-
      Overall store operations — manages P&L, approves restock orders, handles
      staffing decisions, and resolves escalated customer issues
    tags:
      - lead
      - management
      - retail
  ...
  - id: inventory-clerk
    ...
    role: >-
      Inventory management — conducts daily counts, tracks stock levels,
      generates restock orders, and manages supplier communications
    tags:
      - inventory
      - retail
    communities:
  ...
  - id: cashier-lead
    name: Cashier Lead
    role: >-
      Floor operations — manages cash registers, handles customer complaints,
      trains new cashiers, and maintains store presentation
    tags:
      - retail
      - floor
      - customer-service
...
groups:
  - name: Inventory
    description: 'Stock levels, restock orders, and supplier communications'
    community: Store Team
  - name: Floor Operations
    description: 'Customer service, cash handling, and store presentation'
    community: Store Team
  - name: Status
...
workflows:
  - id: kickoff
    name: Team Kickoff
    description: Initialize the store team and assess current operations
    schedule: manual
    enabled: true
  ...
  - id: daily-inventory
    ...
    description: Morning inventory check and restock planning
    schedule: 0 6 * * *
    enabled: true
  ...
  - id: restock-alerts
    name: Restock Alerts
    description: Check for critical low-stock items throughout the day
    schedule: 0 */4 * * *
    enabled: true
    executionMode: automated
    targeting:
  ...
  - id: shift-scheduling
    ...
    description: Weekly shift schedule creation
    schedule: 0 18 * * 0
    enabled: true
    executionMode: managed
    targeting:
