...
- **Public Lifecycle Skeleton** — the public Lifecycle plugin establishes a read-only agent/workflow deep-inspection surface for overview, activity, artifacts, and configuration history without bundling enterprise policy, evaluation, or optimization products.
...
- **Public Lifecycle Plugin** — add a unified read-only timeline, artifact and output inventory, configuration-change evidence, source links, and redacted summary export for one selected agent or workflow.
- **Plugin Manager** — add a compact Plugins settings action that lists discovered plugins and lets authorized users enable or disable compatible plugins while preserving state and explaining restart requirements.
